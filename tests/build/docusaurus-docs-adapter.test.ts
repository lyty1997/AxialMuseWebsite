...
function createPresetHarness(
  ...
  const session = Object.freeze({
    docsAdapterSession: createSession(mode, []),
  });
...
test("E-009 preview preset 只在 current docs version 注入 noIndex=true", async () => {
  const harness = createPresetHarness(undefined, "preview");
  await runPreset(harness.preset, {blog: false, sitemap: false});
  const docsOptions = harness.calls[0]!.docs as Readonly<Record<string, unknown>>;
  assert.deepEqual(Object.keys(docsOptions).sort(), [
    "includeCurrentVersion",
    "onlyIncludeVersions",
  ...
  assert.deepEqual(docsOptions.versions, {current: {noIndex: true}});
  assert.equal(Object.isFrozen(docsOptions.versions), true);
  assert.equal(
    Object.isFrozen((docsOptions.versions as {current: object}).current),
    true,
  );
});
