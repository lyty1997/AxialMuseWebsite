...
- 首次访问者能识别“Axial Muse”，理解“轴心 + Muse”的品牌意象和“用全栈技术 + AI，让所有人用上好用的工具，实现生产力平权”的愿景，同时明确当前公开内容仍以真实项目和技术复盘为依据。
...
- 不提供独立搜索路由、服务端全文索引、日期筛选、主题页、作者页、系列页、归档页、分页、RSS、独立 `/about/` 页面或内容后台；页头纯本地搜索只匹配同批公开安全展示投影。
...
| 路由 | 来源 | M0 行为 |
|---|---|---|
| `/` | `src/pages/index.tsx` | 品牌愿景、精选项目、踩坑记录与关于摘要 |
...
- 第一层显示原生 SVG Logo 与文本品牌“Axial Muse”，共同链接 `/`；右侧为标注清晰的本地搜索输入。
- 第二层固定为带内联 SVG 小图标的“首页”“项目介绍”“踩过的坑”，分别链接 `/`、`/projects/`、`/writing/`，并以 `aria-current="page"` 表示当前栏目。
- 搜索只消费 `projectNavigation` 和 `writingNavigation` 中 `published`/`archived` 项目的标题、摘要与规范路径；即使在 development preview，`draft`/`planned` 也不进入结果。查询仅保存在当前 React 状态，不发送、不持久化、不写 Cookie，结果直接链接既有 canonical 页面。
- 不展示登录和注册按钮。它们不是禁用态占位；账户能力以后必须先确定用途、字段、存储、保留删除、隐私和服务边界。
- GitHub 个人主页与邮箱放在首页关于区；GitHub 地址固定为 `https://github.com/lyty1997`，邮箱固定为 `lyzimin@outlook.com`。
- 360 px 起保持品牌、搜索和三项导航可达、可折行且无页面级横向滚动；不再依赖栏目抽屉维护第二套导航状态。

### 首页

首页是主站入口，不是虚构能力的营销落地页。首屏 H1 为“用全栈技术 + AI，让所有人用上好用的工具。”，并以“让生产力不再是少数人的特权”说明生产力平权愿景；品牌含义区说明 Axial Muse 从轴心时代的大师与 Muse 获得命名灵感，希望通过技术提供比肩大师的工具服务。该表达必须与“项目持续构建中”的真实阶段并置，不得暗示所有服务已经上线。

首屏不占满视口，在常见 800-900 px 高度中完整呈现品牌愿景与品牌含义。首页不再放置项目或文章 CTA；`/projects/` 与 `/writing/` 只通过页头第二层标签进入。页面不把首屏放入卡片，不使用装饰光斑、通用图库图或与项目无关的背景图。

首页后续区域只保留 `#about` 关于，不重复投影“项目介绍”或“踩过的坑”目录、内容卡片与空状态。项目和文章只在各自目录页投影；未发布内容仍不得进入 production。

没有公开项目时，首页与项目目录统一显示：“当前还没有完成公开审核的项目。项目资料通过事实、隐私和视觉证据检查后会在这里出现。”该状态只消费公开导航投影，不得读取或点名 `planned`、`draft` 项目。

关于正文固定为：“我是一个全栈工程师，覆盖人工智能、系统架构、底层驱动、硬件设计、机械工程、制造工艺，曾在达摩院做系统开发。关注 AI 工程、前沿科技，正在进行多个个人项目开发。本站分享公开项目、技术取舍与复盘，不公开凭证或私有仓库。”下方显示邮箱与 GitHub 的图标链接。微动效只使用 CSS，不能妨碍键盘焦点或 reduced-motion。
...
左栏显示项目目录，正文展示项目状态、问题、架构、关键取舍、限制、证据链接与相关技术分享，右栏显示当前正文 H2-H3 标题导航。页面标题、摘要、仓库和状态从注册表投影，正文不得覆盖。相关技术分享只按 `relatedWriting` 的显式顺序显示同批已公开文章的标题与规范链接；空数组不制造区块，未公开或悬空目标失败关闭。

#26 为闭合已登记主预览的 production SSR 引用，只从同一已验证投影输出一张无样式、无附加文案的图片；该工程闭包不代表项目详情表现完成。#27 已在框架默认表现上完成页面结构、完整公开文案、安全关系链接和避免重复渲染；#28 已由提交 `255fe98f494344aca400d5a48a5d19ccb425183d` 完成主题令牌、三栏断点、原生折叠与响应式/可访问性基线，精确 CI run `30244831021` 成功。关闭后审查修复 `6345bb1703000f2f72bcf4a0e71e9b960cf430ab`、`e04085b837c93db9f639ab014ad99c829b270194` 及精确 CI runs `30266019758`、`30270312057` 又收口图片优先级/失败布局、空目录、样式优先级、无 hydration 与移动导航键盘路径；最终 production fixture 已由 Chrome `150.0.7871.125` 连续两次通过 11 项探针和 8 份一致截图摘要。#32 已实现共享 TypeScript/test/build 的双 Node CI 接线；与 #28 浏览器入口合并后的远端执行仍须由 `dev` 精确 SHA 证明。
...
左栏显示生成的技术分享目录，中栏展示正文，右栏显示 H2-H3 标题导航。页面显示作者、`publishedAt`、`updatedAt`、主题与归档状态；UUIDv7 articleId 不作为主视觉信息，不参与公开日期排序。相关项目与相关文章只按 `relations.projects` / `relations.articles` 的显式顺序显示同批当前可见目标的标题与规范链接；空数组不制造区块，production 的未公开或悬空目标失败关闭。关联内容不做浏览器端推荐或行为采集。
...
M0 使用 Docusaurus classic/Infima 的语义、导航和可访问性基础，通过 `themeConfig`、`src/css/custom.css`、CSS custom properties 与页面 CSS Modules 适配品牌。只提供亮色主题，不引入 UI 库，不 eject 主题，不做默认 swizzle；只有现有配置点无法满足已确认需求时，才对锁定版本做最小包装并补契约测试。#28 的 fit-gap 已将布局包装边界锁定为同路径 `DocRoot/Layout` 与 `DocItem/Layout`：内容目录不得进入框架全局移动导航抽屉，标题目录只消费当前正文已有 H2/H3，空目录省略，移动折叠使用原生 `details`。关闭后复核另证明锁定框架缺少全局抽屉 Escape 路径，因此同路径 `Navbar/Layout` 只在 hydration 后调用框架原关闭按钮并归还 toggle 焦点；导航 DOM、内容和状态仍由 Docusaurus 拥有。

### 视觉基线

界面定位为安静、紧凑、可重复使用的工程作品索引。D-147 将品牌视觉统一为蓝色与米黄色：米底承担低反光页面环境，暖白表面承载正文，深品牌蓝建立识别，交互蓝只用于链接、焦点和选中状态。禁止营销式超大口号、装饰性卡片堆叠、模糊光斑、过量阴影和单一色相铺满页面。

D-148 的原生 SVG 标志以两座仰视书架构成开放顶部的 A，以中下部米金弧形流星作为横梁，以同一立体插画语言绘制铺满背景的银河繁星。正式资产按 `clean-vector-redraw` 流程，以颜色分层、连通轮廓提取、平滑和 RDP 简化忠实重绘确认原图，不用手工简化改变构图、材质或细节；清理后不包含星盘、星座、嵌入位图、外部引用、脚本、字体、生成器或编辑器元数据、注释、隐藏图层或不可见追踪图形。导航和 favicon 继续消费同一登记资产。

D-149 覆盖上述 SVG 交付形式：正式标志使用保留确认原图完整像素的 1254 × 1254 真彩 PNG，只删除 `caBX` 及关联非图像元数据。净化工具必须证明解码 RGBA 像素逐像素不变，最终文件只包含 `IHDR`、`IDAT` 与 `IEND`，没有元数据块和尾随字节。

D-150 覆盖 D-149 对旧位图构图的选择：正式标志改为用户本次选定的 1254 × 1254 方形 PNG，固定显示深蓝星空、金色天体网格、组成开放 A 的两座金木色书架式大门，以及穿过中部并在右上形成星芒的青蓝与米白弧带。源文件已经只包含 `IHDR`、`IDAT` 与 `IEND`，没有元数据块或尾随字节，因此正式资产保持原始压缩字节与解码像素。

D-151 以用户再次选定的 1254 × 1254 方形 PNG 覆盖 D-150 正式字节：两座书架主体改为更明亮的米金色，既有星空、天体网格、开放 A、弧带与星芒构图保持不变。源文件仍只包含 `IHDR`、`IDAT` 与 `IEND`，因此直接保留原始压缩字节与解码像素，不改变品牌资产路径或页面布局。

| 令牌 | 基线值 | 用途 |
|---|---|---|
| `--am-canvas` | `#f4eddf` | 页面米黄色底 |
| `--am-surface` | `#fffcf6` | 暖白内容表面 |
| `--am-ink` | `#16242e` | 主文本 |
| `--am-muted` | `#59636b` | 辅助文本 |
| `--am-line` | `#d8ceba` | 装饰分隔 |
| `--am-line-strong` | `#938772` | 控件和重要边界 |
| `--am-brand` | `#153a5b` | Logo 主面和品牌识别 |
| `--am-accent` | `#2b6995` | 链接、焦点和选中状态 |
| `--am-accent-soft` | `#e5edf2` | 悬停与浅色状态面 |
| `--am-signal` | `#985932` | 少量状态警示 |
...
文件名使用小写 ASCII 与连字符。`static-public/` 的每个文件还必须在 `docs/contracts/static-public-assets.json` 以 `brand` 或 `operational` 角色逐项登记，目录与登记必须一一对应；D-151 当前只登记 `assets/brand/axial-muse-mark.png`，作为页头 Logo 与 favicon，不以占位素材填充。生产构建只从全新的临时静态树读取已登记 `static-public/` 与 `published`/`archived` 项目白名单；预览构建可包含所有状态中已登记的项目预览，但不能成为交付输入。生产制品中的项目资源必须与白名单逐路径、逐字节一致，未发布素材的路径或字节不得泄漏。生产页面不引用仓库外图片 URL。截图必须移除凭证、真实隐私、用户名、主机名、绝对路径、通知和未授权内容；预览隔离不是保密边界，版权与公开性无法确认的素材不得进入 Git。
...
- 全站 `lang="zh-CN"`，UTF-8；首页标题为 `Axial Muse | 全栈技术 + AI 的生产力工具`。
- 项目目录标题为 `项目介绍 | Axial Muse`，技术分享目录标题为 `踩过的坑 | Axial Muse`；项目与文章详情标题统一为 `<内容标题> | Axial Muse`。
- 首页 description 为 `Axial Muse 以全栈技术与 AI 构建好用的工具，分享公开项目、技术取舍与工程复盘。`
...
- 品牌愿景、关于、邮箱、GitHub 主页和 ICP 备案链接符合本 Spec；GitHub 主页只在关于区出现一次，页脚不重复。
