...
D-035 固定项目与文章 slug 规则：

- 由作者手工填写能够表达内容语义的英文 slug，不从中文标题、日期、分类或系列自动生成。
- 只允许小写 ASCII 字母、数字和连字符，不使用中文、拼音自动转换、空格或下划线。
- 发布前完成确认，发布后保持稳定。
- 确需修改时，为旧 URL 配置永久重定向并同步 canonical URL 与站点地图。
- 项目 slug 逐项目登记；例如 DocRestore 使用 `docrestore`。
D-038 进一步确认技术文章必须在 frontmatter 顶层显式填写 `slug`，该字段是文章公开 URL 的唯一真相源。文章文件名和目录不参与路由生成；构建门禁检查格式、全站唯一性以及已发布 slug 变更对应的永久重定向记录。
...
D-055 进一步确认技术文章的领域内容模型是唯一可编辑真相源，Docusaurus 页面元数据只能在构建期单向投影。项目内容来源仍需另行确认；共用 docs 实例不表示把当前项目注册表字段直接复制到项目 frontmatter，也不允许为框架便利给文章增加语义重复字段。

D-056 进一步确认技术文章通过 Docusaurus 官方 `markdown.parseFrontMatter` 调用仓库内纯投影函数，只转换本次构建使用的内存元数据。逐字段投影与错误契约仍需确认；该决定不改变页面职责，也不替项目介绍选择内容源或适配方式。

D-057 进一步确认相对未来 docs 内容根的 `writing/` 子树是唯一技术文章类型边界。该目录不参与公开 URL、侧栏归属或排序，文章仍以源 `slug` 形成 `/writing/<article-slug>/`；目录外内容不自动解释成项目介绍。内容根物理路径、内部组织、误放检测、逐字段投影和项目适配仍需确认。
...
> 状态说明：D-036 至 D-050 已确认技术文章核心字段、分组式 frontmatter、受控 SEO 描述覆盖、作者注册表、不设主分类的“项目-模块-主题标签”组织规则，以及必填的 `classification` 唯一分组；D-055 确认这些领域字段是唯一可编辑真相源，框架字段只作构建期投影；D-056 确认技术文章通过官方 `markdown.parseFrontMatter` 与本地纯投影函数完成内存适配；D-057 确认相对内容根的 `writing/` 子树是唯一技术文章类型边界。下面的 Project、Writing 与 Series 字段表形成于这些决定之前；除已确认的 Writing 字段和组织语义外，目前只作为下一轮评审的候选输入，不是最终 schema 或实现契约。各注册表结构与路径、通用分组名称、跨项目相关关系字段、摘要精确长度、其余字段、项目模型、内容根物理路径和逐字段投影仍需用户逐项确认。
