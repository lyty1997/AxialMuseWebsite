...
D-056 曾确认技术文章通过 Docusaurus 官方 `markdown.parseFrontMatter` 调用仓库内纯投影函数。D-058 证明 `slug` 可以直接使用原生字段后，用户要求先完成其余字段的官方能力 fit-gap，再重新决定是否需要适配；因此 D-056 当前只保留历史方案，实施授权暂停。

D-057 曾把相对未来 docs 内容根的 `writing/` 子树设为技术文章类型边界，并与 D-056 的全局解析分流绑定。该决定已随适配方案重新开放评审；在重新确认前，不创建该内容目录或类型分流。无论后续选择何种内容组织，文章文件路径都不得生成或覆盖 D-058 的原生完整 `slug`。
...
> 状态说明：D-036 至 D-050 已确认技术文章核心字段、分组式 frontmatter、受控 SEO 描述覆盖、作者注册表、不设主分类的“项目-模块-主题标签”组织规则，以及必填的 `classification` 唯一分组；D-055 确认这些领域字段是唯一可编辑真相源；D-058 确认文章 `slug` 直接采用 Docusaurus 原生完整路径。D-056、D-057 的适配执行点和内容类型判据已经重新开放评审。下面的 Project、Writing 与 Series 字段表形成于这些决定之前；除已确认的 Writing 字段和组织语义外，目前只作为下一轮评审的候选输入，不是最终 schema 或实现契约。各注册表结构与路径、通用分组名称、跨项目相关关系字段、摘要精确长度、其余字段、项目模型、内容根物理路径、原生字段 fit-gap 和最小适配范围仍需用户逐项确认。
