...
- **CI 纠错证据**：首个实现提交 `72c7c200aeea2db5eb027dc568c272565b36d0ee` 的 [CI run 30346480458](https://github.com/lyty1997/AxialMuseWebsite/actions/runs/30346480458) 在双 Node 端点的独立 TypeScript 门禁暴露了 Docusaurus 3.10.2 不支持根 `generatedFilesDir` 配置；修复提交 `c468def300d87f650692660cdf0c6faddfea3607` 改用官方环境变量后，[CI run 30348715402](https://github.com/lyty1997/AxialMuseWebsite/actions/runs/30348715402) 的双端点 typecheck 均通过，但完整测试又暴露 `build --dev` 自动启用 classic debug 页面，这些页面不满足 preview 全路由 noindex 闭包。当前纠正版显式 `debug: false`，并同步关闭真实 `planned` 项目的路由、导航和可选图片投影缺口；这两次失败均保留失败关闭，没有跳过或放宽检查。
- **当前验证与遗留**：`git diff --check`、WSL `bash -n scripts/dev/preview.sh`、严格 TypeScript 和 39/39 模块边界 fixture 已通过；任务私有冻结依赖副本在精确 Node `24.18.0` 上完成 252/252 测试，其中真实 production/preview Docusaurus fixture 均成功，WSL 无 Chromium 的两处调用只在诊断副本中用受控回执替代，未冒充真实浏览器证据。随后以仓库当前两个真实 `planned` 项目构建 worktree 外 preview 候选并通过独立制品验收。Windows 工作区的 WSL Ubuntu 使用本轮临时安装的官方 nvm `0.40.6` 和精确 Node `24.18.0`，未创建默认 alias；当前纠正版仍须通过最终 pre-commit、提交后双端点 CI 与真实 GitHub Chromium 回归。Linux 托管机实际 gitignored `PREVIEW_STATE_DIR` 迁移、冻结依赖准备和正常/失败/并发/PID/端口/回滚现场验收也仍未完成，因此不能标记 #8 完成；Issue 写回仍需另行授权。
