...
- **后续门禁**：topic 身份修正与发布门禁已关闭，当前正在按 D-136 保留 #8 语义完成 `dev` 本地集成；`dev` push CI、`dev -> main` PR/main CI 和 canonical `main` artifact 仍未取得。服务器、云资源、Issue、verifier、后续 evidence 晋级与 #37 仍未授权。
...
- **本地集成与纠错**：专题 `e88386b` 已与 `origin/dev` 的 #8 普通合并，六个文本冲突按既定边界同时保留 preview、production release、verifier、producer 与受限 dispatch 语义；自动合并的 JavaScript 明列、统一质量、模块边界、seal 和插件热点也经独立复审确认两侧覆盖完整。合并门禁先暴露调用者 `umask 0002` 会让 Docusaurus 生成组可写文件，因此构建入口现在只在同步 Docusaurus 子进程窗口固定 `0022` 并在所有路径恢复调用者原值，不放宽后续 owned-file 检查；权限根因关闭后又暴露 dev bundle 把 config chunk 映射放入 `eval` 字符串，闭包证明现同时精确接受未转义与转义 chunk key，但仍要求同一 `main.js` 内存在真实 config 模块头和对应动态导入，不能仅凭路由映射补造文件。
- **本地证据与剩余门禁**：精确 Node `24.18.0` 已通过 fresh 隔离冻结安装、完整零依赖 `quality`、E-013 完整历史、严格 `typecheck`、16 个 TypeScript 源的 255/255 测试（含真实 production/preview Docusaurus 与 Chrome 回归）、production `build`；系统 Python 3.12 的 verifier/bootstrap 为 43/43，`git diff --check` 通过。config-chunk 定向测试另在 `umask 0002` 下为 18/18，并断言真实 preview `main.js` 无 group/other write 位。`package:artifact`/`check:artifact` 仍须在本地 merge commit 形成干净精确 SHA 后执行；`dev` 普通 push、该 SHA 的 CI、`dev -> main` PR/checks、`main` CI 和真实 artifact 仍未发生，服务器与云资源保持未写入。
