...
## 2026-07-29 — #35 服务器侧独立 artifact verifier（本地实现与完整本地验收完成）
...
- **遗留与外部边界**：当前 #35 改动尚未提交或推送，也没有远端 CI 或 Issue 回填。#14 固定 upload Action 后仍须以真实下载 ZIP 复验 archive shape；#36 仍须在目标 Ubuntu 核验 `/usr/bin/python3`、安装副本的 root ownership/mode 与系统能力；#37 必须在部署锁内重新验证 staging 身份和整树摘要后才可安装。未读取 GitHub 凭证，未下载真实 artifact，未操作服务器、TAT、Nginx、DNS、TLS、账本或生产目录，也未处理用户数据。
