...
- **当前验证与遗留**：`git diff --check`、WSL `bash -n scripts/dev/preview.sh`、严格 TypeScript、39/39 模块边界 fixture 和新增 config-chunk 正常/反例 fixture 已通过；任务私有冻结依赖副本在精确 Node `24.18.0` 上完成 252/252 测试，其中真实 production/preview Docusaurus fixture 均成功。Windows Node `24.15.0` / Chrome `150.0.7871.125` 已对同一 production 候选通过全部 11 个浏览器探针，并复现 preview 唯一的 config chunk 404；给缺失且已证明合入 `main.js` 的 config chunk 写入确定性最小登记后，同一 preview 候选的 11 个探针全部通过。当前仓库入口已把这项 Docusaurus 3.10.2 兼容处理放在 Docusaurus 成功与候选证据捕获之间，已有文件不覆盖，映射、合并证据、路径、权限或字节任一异常均失败关闭。Windows 工作区的 WSL Ubuntu 使用本轮临时安装的官方 nvm `0.40.6` 和精确 Node `24.18.0`，未创建默认 alias；当前兼容修复已在精确 Node `24.18.0` 通过完整 pre-commit，最终组合树 `dev` CI 与 GitHub Chromium 回归仍待。Linux 托管机实际 gitignored `PREVIEW_STATE_DIR` 迁移、冻结依赖准备和正常/失败/并发/PID/端口/回滚现场验收也仍未完成，因此不能标记 #8 完成；Issue 写回仍需另行授权。
