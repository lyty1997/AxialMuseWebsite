...
当前 `public/` 仍是迁移前公开入口，但仓库的 Docusaurus production build、#13 重定向派生、#33 `payload/` + `metadata/` release 封装/复验和 #35 服务器独立 verifier 已经具备；#14 也已在当前工作区把 CODE-015/CODE-019/CODE-020 入口接入 `production-artifact` 的 fresh build 与单次上传。本地生成的 `dist/release/` 不是已上传 Actions artifact，本地 verifier 与 workflow fixture 通过也不是 canonical `main` 真实 producer run 或目标服务器验收，不能据此声明实际 artifact ID/digest/ZIP 或生产发布完成；#36 须固定并核验服务器安装副本和系统运行时，#37 才消费 `verified-release`。生产服务器最终只安装已验证 payload 和非公开运行配置，不解释源注册表、不拉取源码、不运行 Node/npm 或从源码 checkout 执行脚本，也不成为内容编辑或构建源。
