...
| 决策域 | M0 选择 | 设计依据 |
|---|---|---|
| 页面技术 | 当前公开交付仍是手写静态骨架；仓库已建立 Node 24 LTS + 严格 TypeScript + Docusaurus `3.10.2`，#26 的唯一 `site-content/` docs 实例、内容投影和 production 候选制品验收已进入当前精确基点并完成远端闭环 | classic/Infima 最小适配；首页与列表页使用严格 TypeScript 页面，项目与文章详情由 docs 实例承载；项目/文章双侧栏遵守 E-004 的三档响应式契约，不引入 UI 库、搜索或文章专属交互 |
...
| 当前质量运行时 | 仓库已固定 `.nvmrc` 精确基线与 `>=24.16.0 <25` 兼容入口；D-080 已让本地 pre-commit 在子进程使用用户级 nvm/Node 24，系统默认 Node 仍为 Node.js 22 ESM；D-097 至 D-099 已在工作区把 Ubuntu CI 迁移到 Node 24 主/最低端点 | E-010 隔离 runner、随附 npm 双端点、离线 CLI、本地作者 hook 与 #21 真实图准入已落盘；Node `24.18.0`/npm `11.16.0` 与 Node `24.16.0`/npm `11.13.0` 已对同一 lock 完成隔离冻结安装和工作区 CI 本地验收，尚无提交后的远端运行证据 |
| 图表 | 现有 PlantUML 源码编译为静态 SVG | 保留构建期图表流程，不增加浏览器端渲染或 Docusaurus 运行时插件 |
| 质量与供应链 | D-053、D-074、D-077、D-079 固定能力类别、独立 `tsc --noEmit`/Node ESM test/Docusaurus build 和 npm 原生失败关闭准入；D-099 使普通 CI 只执行静态供应链证据，不联网 audit | #9/#10 已实现 E-010/E-011；#21 已完成 1,225 个 canonical identity 的真实 tarball 审查与准入、正式 SBOM/evidence/NOTICE、首次准入当时的 audit 全零、D-082 最终决定和双端点 composite receipt；2026-07-26 最新 live audit 的 18 个 high 仍是未修复风险。#22/#11/#26 已接通 typecheck、测试与真实 production build，D-097 至 D-099 已在工作区接通 Node 24 CI、完整历史和静态供应链门禁；远端 CI、release 封装和部署门禁仍待后续阶段 |
...
- 根 `docusaurus.config.ts` 与 `sidebars.ts`：D-075 的框架入口由 #22 创建；#26 已以 E-016 的 classic-derived preset 接通唯一 `site-content/` docs 实例，并显式导出只消费已校验投影的 `projectsSidebar` 与 `writingSidebar`。production 零公开正文只跳过固定版本已证实有缺陷的官方 `contentLoaded`；有公开正文时对受控浅投影委托并清空其 `drafts`，防止框架把未发布 ID 写入浏览器数据；不保留 `docs:false`、占位正文或第二内容根。
...
- `src/domain/` 与 `src/build/`：领域核心与构建期适配源码；#22 已创建 `src/build/site-config/`，#23 已创建 `src/domain/content/`，#26 已增加路由/侧栏/日期索引纯投影与 `src/build/content/` 的扫描、frontmatter、Docusaurus 和制品适配。`src/build/` 是源码目录，不是静态构建产物目录。
...
- `scripts/build/`：#26 已把 #22 的入口收束为 production 独占构建事务：先构建任务私有候选并由独立 Docusaurus 进程验收，锁内可回滚切换到 `build/` 后再以 fresh session 验收终态路由、canonical、sitemap、真实侧栏、私有日期索引和 E-008 字节闭包。上一版或失败候选只能进入固定 retired/quarantine，成功释放锁才是 commit point；这不宣称 POSIX 单 syscall 目录交换。preview 的持久候选与激活仍由 #8 接管。
