...
本文记录 D-027 至 D-083、D-097 至 D-100 的已确认、替代与重新评审关系，并引用依 D-078 形成的 M0 工程决定：
...
- 已通过 D-079/E-012 固定 `@types/node` 的直接开发依赖、独立 NodeNext 测试 program、临时 ESM emit、运行时 `.js` 说明符、`node --test` 和双 Node 端点同负载；依赖已随 #21 准入，配置、runner 与 fixture 已由 #11 实现并远端闭环，D-097 至 D-099 已完成目标 Node 24 CI 迁移与本地验收，并依 D-100 纳入专题分支；该分支尚未推送或取得远端运行结论。
