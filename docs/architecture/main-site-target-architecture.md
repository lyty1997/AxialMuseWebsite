...
- D-035：项目与文章采用手工英文语义 slug，发布后保持稳定，改名时永久重定向旧 URL。
- D-036：技术文章采用完整编辑模型，覆盖完整的编辑、关联、SEO、推荐与公开修订能力。
- D-037：技术文章采用分组式 frontmatter，正文与元数据保持单文件，复杂可选元数据按职责嵌套分组。
- D-038：技术文章在 frontmatter 顶层显式保存 `slug`，并以它作为公开 URL 的唯一真相源。
...
- D-055：领域内容模型保持唯一可编辑真相源，Docusaurus 元数据只在构建期单向投影，不回写或持久化语义副本。
- D-056：技术文章通过 Docusaurus 官方 `markdown.parseFrontMatter` 调用仓库内纯投影函数，只转换本次构建的内存元数据。
- D-057：相对未来 docs 内容根的 `writing/` 子树是唯一技术文章类型边界，不新增类型字段或独立成员清单。
...
内容根目录、`routeBasePath`、侧栏文件与键名、项目和文章如何关联具体侧栏、侧栏生成与排序、`/projects/` 与 `/writing/` 列表页的生成方式，以及 Docusaurus 逐字段投影的具体契约仍由后续决策确定。本决定不授权安装依赖或配置 preset。

## 内容字段适配边界

D-055 固定内容模型与框架之间的真相源方向，不固定具体映射表。技术文章继续以 D-038 至 D-050 已确认的领域字段和 Git 注册表作为唯一可编辑来源；Docusaurus 需要的等价或派生元数据只能在构建期从这些输入只读直传或生成。派生结果不回写 Markdown/MDX、注册表或其他已提交文件，Docusaurus 构建产物也不成为编辑来源。

D-056 进一步固定技术文章适配的执行位置：Docusaurus 全局 [`markdown.parseFrontMatter`](https://docusaurus.io/docs/api/docusaurus-config#markdown) 先调用框架提供的 `defaultParseFrontMatter`，再把解析结果交给仓库内的纯投影函数，并把函数返回的 frontmatter 与未修改正文继续交给本次构建。投影不得写文件、回改源 frontmatter、生成临时规范化内容树或通过自定义内容插件另建一条内容加载管线。该函数的输入输出契约、模块位置与实现语言仍随 schema 和构建契约确认。

D-057 固定该全局扩展点的技术文章分流边界：未来单一 Docusaurus docs 内容根内的 `writing/` 子树是唯一技术文章集合。这里的内容根是未来提供给 Docusaurus docs 内容插件的站点内容目录，不是仓库现有的设计文档 `docs/` 目录，其物理路径仍需另行确认。实现必须先把 `filePath` 规范化并验证其确实位于该子树内，再对其中的 `.md` 与 `.mdx` 文件调用技术文章纯投影函数；不能用模糊字符串前缀把相似目录误判为文章目录。子树内每个候选文件都必须按技术文章领域 schema 校验；缺失或无效字段时中止构建，不得退回普通文档。子树外文件继续使用默认解析结果，不因“不属于技术文章”就自动成为项目内容。schema 是否拒绝未知额外字段仍由后续错误契约决定。

该方向受以下边界约束：

- 作者只维护本站领域字段，不为框架便利再写 `draft`、`description`、`tags`、`date` 或第二套作者资料。
...
- `writing/` 只承担领域类型识别，不生成或覆盖 URL、slug、侧栏归属、分组或排序。文章无论在该子树内如何组织，公开地址仍只能由源 `slug` 投影为 `/writing/<article-slug>/`；内部移动不得改变 URL。
- 作者不填写 `contentType` 等第二种类型标记，也不维护文章成员清单。docs 内容根的物理路径、`writing/` 内部目录结构、误放检测细节和项目内容来源及适配方式仍需后续确认。
- 逐字段映射与回退、schema、错误契约、契约测试和错误定位方式仍需后续确认。若官方扩展点无法保持既定语义，必须暂停并重新决策，不能静默切换到临时内容树或自定义内容插件。
...
Docusaurus 的单一 docs 内容拓扑、领域字段向框架字段的构建期单向投影方向、技术文章的官方适配执行点和相对内容根的 `writing/` 类型边界已经确认；具体版本、preset、实例配置、内容根物理路径、逐字段投影、错误契约、项目适配、内部内容目录、构建命令、产物目录、构建发生在 CI 还是发布主机，以及依赖锁定方式尚未确认。实现前必须完成对应设计，不得把 Docusaurus 的工具默认值当作用户决定。
...
| 范围 | 已固定职责 | 仍受门禁约束 |
|---|---|---|
| 站点与内容 | 使用 Docusaurus 官方且通过依赖准入的静态构建能力承载文档站；项目介绍与技术文章共用单一 docs 内容实例并分别使用各自侧栏；领域内容模型保持唯一可编辑真相源；相对内容根的 `writing/` 子树由官方 `markdown.parseFrontMatter` 与本地纯投影函数按技术文章 schema 处理 | 具体版本、preset、实例配置、内容根物理路径、逐字段投影、错误契约、项目适配和主题适配仍由后续决策确定 |
...
| 数据类型 | 目标真相源 | 当前状态 |
|---|---|---|
| 技术文章、项目介绍、页面文案 | Git 仓库内的受审内容 | 文章核心字段、日期、组织基数、`classification` 唯一分组和相对内容根的 `writing/` 类型边界已确认；完整 schema、内容根物理路径、`writing/` 内部组织和项目内容位置尚未决定 |
...
- 项目和文章 slug 由作者手工填写，不从标题、日期、分类或系列自动生成。
- 技术文章必须在 frontmatter 顶层显式填写 `slug`；该字段是 `/writing/<article-slug>/` 的唯一 URL 真相源。
...
- `publicationStatus`、`summary`、日期、作者与 `classification` 到 Docusaurus 原生 front matter 的逐字段投影与回退、纯投影函数契约、误放检测和错误行为；具体实现确认前仍不得要求作者双写 `draft`、`description`、`tags`、`date`、`contentType` 或第二套作者资料。
...
- 已明确技术文章使用顶层显式 `slug` 作为公开 URL 的唯一真相源，文件路径不参与路由生成。
