...
#22 的 I-04 基线尚无可被真实 schema 验证的 Markdown/MDX，固定版本 docs 插件又拒绝零文档实例，因此当前 classic preset 暂时显式设置 `docs: false`，只验证框架、严格 TypeScript、模块边界和最小 production build；这不是对目标拓扑的替代。#23 已实现纯内容门禁，#5 已在该门禁后迁移真实项目正文，#6 已完成媒体纯领域门禁的本地验收；拥有单 docs 实例、frontmatter 投影、侧栏与路由范围且仍 blocked-by #7 的 #26 随后必须原子启用下述唯一 docs 实例。不得用占位文档、第二内容根、额外实例或条件性 fallback 绕过零文档事实。
...
- 继续把 E-001 至 E-015 转化为安全扫描投影、路由检查、主题 fit-gap、侧栏/SEO、素材白名单、局域网候选切换、完整 Git 历史、同版本服务端 301、production build 字节闭包、静态构建和 artifact 制品检查；隔离 npm 与 Node 24 双端点已由 #9 验收，确定性 SPDX 已由 #10 验收，Node ESM 测试已由 #11 验收，内容解码与领域核心已由 #23 验收，两份真实项目正文由 #5 验收，媒体纯领域门禁由 #6 完成本地验收，其余由 #7/#8、#12 至 #14 与 #26 跟踪。
