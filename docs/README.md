...
- 阶段：Docusaurus 静态主站的上层方向和 M0 多页面基线已经固定。D-059 至 D-077 约束内容身份、单一 docs 拓扑、Node 24、严格 TypeScript、依赖候选与供应链准入；D-078 授权 Agent 收口不改变既定边界的 M0 内部工程细节，D-079 增加 Node 24 测试类型直接候选，E-001 至 E-015 已固定项目内容职责、URL、注册表、主题适配、静态制品交付、主预览 schema、发布态素材白名单、可验收草稿预览、npm 启动前隔离、确定性 SPDX、Node ESM TypeScript 测试、统一结构化 frontmatter 解码、HEAD 可达完整 Git 历史、与静态 payload 同版本激活的服务端 301，以及 production artifact 自包含重建与字节闭包。2026-07-19 已建立 [M0 Roadmap #15](https://github.com/lyty1997/AxialMuseWebsite/issues/15) 及 #16 至 #20 阶段父任务；#4 按设计一致性职责验收完成并关闭，#5 至 #14 保留原编号并改为实现期单一不变量任务，#21 至 #43 补齐其余实现、内容、发布、上线和维护任务。#9 已关闭并收口 E-010 隔离实现与版本契约；#10 已完成 E-011 确定性 SPDX 实现、真实 npm shape 和双目标端点全量验收。#21 的 D-077 首次真实依赖解析仍须单独申请 npm 联网授权，并继续生成真实 lock/tarball/许可证/NOTICE/审计与准入证据；#22 只消费版本契约和已准入依赖图建立站点/TypeScript 基线。Git 推送与发布、GitHub Action/凭证、服务器、DNS、证书和生产操作也仍需各自授权。
