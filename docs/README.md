...
- 阶段：Docusaurus 静态主站的上层方向和 M0 多页面基线已经固定。D-059 至 D-080 与 E-001 至 E-016 已约束内容身份、单一 docs 拓扑、Node 24、严格 TypeScript、依赖与供应链、静态构建、完整 Git 历史、同版本服务端 301、production artifact 字节闭包、单一内容装配和作者 Linux 运行时。#9、#10、#21、#22、#11、#23、#5、#6、#7 与 #26 已完成各自远端闭环；#27/#28 已完成页面、主题、响应式与可访问性闭环并合入 `main`。#12 历史门禁、#24 直接 Node 作者入口与原子事务、#32 workflow 契约均已完成专题实现和本地验收；D-104 约束三者经 `dev -> main` 晋级，并要求以 `dev` 组合树精确 SHA 的真实 CI 与对应 Issue 证据完成远端验收。#13 已完成仓库侧 301 派生、production payload 校验和固定 digest Docker 真实 Nginx 验收；#33 的确定性 `dist/release/` 封装、规范整树摘要和独立复验已由提交 `b38354b` 推送到专题分支，但该 ref 不触发现有 CI，远端证据仍待；#35 的服务器独立 verifier、共享 golden 和本地负向验收已由本地提交 `f7fdc43` 实现但尚未推送。#14 的 producer/upload 接线已由本地提交 `7b5cc47` 实现，#34 的身份/新鲜度与受限 TAT dispatch 候选已由本地提交 `2c40e87` 实现；两者尚无 canonical `main` 真实上传、实际 artifact outputs/ZIP、活动 TAT 接线或 Issue 关闭证据。#36 已完成服务器侧第一阶段只读盘点、废弃旧站清理和控制面基础核验，单次历史带宽告警的归因不确定性已由所有者按 D-116 接受，当前加固前系统盘快照也已按 D-117 创建并确认成功；额外管理身份已按 D-118 完成可逆禁用并通过独立后验，账户、home、密钥和其他文件均保留；D-119/D-120 SSH 全局策略已经达到 `committed_clean`。D-121 又形成双锁、receipt 身份绑定和失败隔离的 verifier bootstrap 工作区候选，但 bootstrap/#35 尚未进入 canonical `main`，也没有现场安装。OS/腾讯云防火墙、待升级包与维护重启、verifier/Certbot 安装仍为 GAP，#37 不得提前开始。当前两个项目仍为 `planned` 且没有已批准主预览，技术文章为零，production 只能输出可信空状态。
- CI 状态：D-097 至 D-102 已形成第一阶段可信 CI 的专题实现：四个失败关闭 prerequisite job、三个官方 Action 固定 SHA、Node `24.18.0`/`24.16.0` 双端点、完整 checkout、E-010 隔离安装、零依赖 `quality`、安装后 E-013 历史门禁、独立 `typecheck`/`test`/`build` 与静态供应链证据。D-102 把依赖冻结的 E-013 CLI/fixture 从零第三方依赖 `quality` 中拆出，改由两个构建 job 在冻结安装后显式运行，恢复无 `node_modules/` 的本地 quality/pre-commit，同时保持同一 Docusaurus 解析器和双端点失败关闭。D-098 已消除私有构建绝对路径进入公开 JavaScript 的问题，D-099 又从普通 push/PR CI 移除 live npm audit；既有 18 个 high 依赖节点仍由 Dependabot Alerts 与人工依赖维护跟踪，不表示风险已修复或接受。#33 提供仓库侧 `package:artifact`/`check:artifact` 与 release 字节闭包，#35 的 Python 验收按 D-109 保持在 Node-only `quality`/pre-commit 之外；本地提交 `7b5cc47` 已把 D-110 固定完整 SHA 的官方 `actions/upload-artifact`、只允许 canonical `main` push 的非 matrix `production-artifact`、fresh 自包含重建、单次精确 ZIP 上传、跨 Action build/release 操作 seal、HEAD blob 闭包和七项 outputs 接入 workflow。本地提交 `2c40e87` 又在活动 workflow 之外实现七项身份/main 新鲜度复核、无 Secret 预检、固定 TC3 `InvokeCommand` dispatch 与 main 不取消/生产串行的静态候选；`status: dispatched` 只表示取得 invocation ID，不等于部署成功。由于远程仓库暂不可用且 Actions 暂无额度，真实 producer run、artifact ID/digest/ZIP、`main` required checks 仍未取得；#34 的 environment/CAM/TAT 活动接线、#36 的 OS/腾讯云防火墙、维护重启与 verifier/Certbot 安装和 #37 的 TAT 终态、服务器/Nginx/DNS/TLS 也未完成。后续依赖变化、deploy Action、凭证和生产操作仍须分别准入、决策与授权。
