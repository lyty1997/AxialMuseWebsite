...
- 阶段：主站已改选 Docusaurus，并已固定单一 docs 内容拓扑、领域内容单一真相源、根 `routeBasePath` 与技术文章原生完整 `slug`；项目介绍和技术文章共用一个 docs 内容实例、分别使用项目侧栏与技术分享侧栏，不启用 blog。D-059 已完成技术文章核心字段的原生 frontmatter fit-gap，并把 D-056 收敛为原生字段精确直用、其余必要字段通过全局 `markdown.parseFrontMatter` 做只读内存最小适配；D-060 已独立确认未来 docs 内容根内的 `writing/` 子树是唯一技术文章类型边界。后续仍需逐项确认 docs 内容根物理路径、完整 schema 与错误契约、各注册表、侧栏生成、主题与 SEO 元数据组件及标签合并、具体门禁工具和构建发布契约；在这些实现前置项完成前仍不创建解析配置、内容目录或文章文件。
