...
- 阶段：Docusaurus 静态主站的上层方向和 M0 多页面基线已经固定。D-059 至 D-080 与 E-001 至 E-016 已约束内容身份、单一 docs 拓扑、Node 24、严格 TypeScript、依赖与供应链、静态构建、完整 Git 历史、同版本服务端 301、production artifact 字节闭包、单一内容装配和作者 Linux 运行时。#9、#10、#21、#22、#11、#23、#5、#6、#7 与 #26 已完成各自远端闭环；#27 已完成首页、目录/详情公开表达、导航、页脚、统一 SEO 与安全关系链接投影，#28 已完成中性编辑式主题、三档响应式目录、原生折叠、移动导航与可访问性验收，两项均已合入 `main`。#12 历史门禁、#24 直接 Node 作者入口与原子事务、#25 显式文章日期事务、#32 workflow 契约已经纳入 `dev`；[Issue #8](https://github.com/lyty1997/AxialMuseWebsite/issues/8) 的 preview 候选、草稿/noindex/sitemap 制品门禁、冻结依赖证据与原子 current 切换也已在本地实现，并在 WSL Ubuntu 的精确 Node `24.18.0` 通过完整本地质量、历史、类型、production 和真实 preview 构建门禁。上述组合树仍须以最新 `dev` 精确 SHA 的真实 CI 和各自 Issue 证据完成远端验收；#8 还缺 Linux 托管机配置与真实局域网浏览器现场验收，不能标记完成。当前两个项目仍为 `planned` 且没有已批准主预览，技术文章为零，production 只能输出可信空状态。
...
- CI 额度与优化提案：2026-07-28 当前 private repository 的 GitHub Actions 月度额度已经用完，最新 #8 run 未分配 runner；本月不增加支出、不再 push 或重跑。现有四 job CI 的逐 job 取整与重复负载由 [Issue #61](https://github.com/lyty1997/AxialMuseWebsite/issues/61) 跟踪，`open-decisions.md` 已记录分层 CI、本地完整门禁和 WSL self-hosted runner 三个互斥候选，推荐分层方案但尚未得到用户最终确认。确认与实施前，当前 workflow、hook、required checks 和 runner 边界均保持不变。
