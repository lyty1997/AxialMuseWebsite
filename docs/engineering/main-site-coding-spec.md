...
- `validateArticleSource({catalog,sources})` 名称保持既定公共 API，但参数和返回值固定为整批文章源；这是 articleId/slug 唯一性、`relations.articles`、项目 `relatedWriting` 和跨文章 recommendation priority 不散落到适配层的唯一实现。每个 source 携带 `sourcePath`、结构化 frontmatter、正文及上述文件系统事实；函数稳定按 articleId 输出完整成功数组，任一文章或跨文章关系失败时不返回部分 value。UUIDv7、完整 `/writing/<lowercase-kebab-case>` slug、状态日期、作者/主题/项目/模块、关系、SEO 去重、推荐、修订和来源逐项按内容模型校验；draft 禁止 `publishedAt` 与 `revisions`，公开状态要求两日期且 `updatedAt >= publishedAt`。`published|archived` 文章的 `classification.project`、`relations.projects` 与 `relations.articles` 只能指向同为 `published|archived` 的目标，公开项目的 `relatedWriting` 也只能指向公开文章；draft、planned 等未发布源仍可引用已经登记且通过自身 schema 的目标。重复 articleId、错误所有者或状态冲突使整个批次失败，不能借重复身份或无部分 value 语义绕过可见性闭包。所有要求 string 的日期值在 YAML 中必须写成带引号的 scalar；固定 Docusaurus parser 会把未加引号的 YAML timestamp 解码为 `Date`，该非 plain 值在结构化快照边界失败，不得静默转为 ISO 字符串。首次发布日期不可改和稳定 ID 历史不可复用仍由 #12 负责；保留路由与全站冲突由 #26 负责。
...
- I-12 对文件数量、深度、路径字节、单文件和总快照/制品字节设置固定上限，并按 UTF-8 path bytes 确定性排序；这些是持续实施契约，不因未逐项执行全部边界 fixture 而宣称已经验收。当前已执行 fixture 使用不同字节的 published/archived/draft/planned 静态 WebP，证明双模式集合与输出隔离、乱序等价、源替换后仍写入原快照、`static-public` 登记闭合和保留 namespace，以及 production 素材缺失、多余、改字节、无真实 SSR 引用、未发布 path、跨 chunk 改名同字节和通用 draft 文章素材泄漏。#7 只建立 production/preview 双模式静态素材计划、受控构建上下文、私有临时树和上述目的限定 checker；不把内容装入 Docusaurus，也不使 preview Docusaurus 可用。#26 独占可供两种模式共享的内容扫描与投影 API、唯一 docs 实例基础装配及 production Docusaurus 生命周期接线；#8 消费 #26 的共享投影和 #7 的 preview 素材计划，独占 `build --dev`、`noindex`/sitemap、持久候选与原子激活；#33 独占 release 封装、规范 wire format 和整树 release digest。
