...
状态：active
完整度：M0-design-closed（#9/#10/#21/#22/#11/#23/#5/#6 已完成各自实现与远端验收；#7/#8、#12 至 #14 及后续装配任务继续跟踪）
最近更新：2026-07-23
...
2026-07-18 审查确认的实施契约矛盾由 GitHub Issues #5 至 #14 跟踪。E-006 至 E-015 已补齐内容所有权、主预览、发布态素材、草稿预览、npm 隔离、SPDX 确定化、Node ESM 测试、完整 Git 历史门禁、同版本服务端 301 和 production job 字节所有权；#9 已完成 E-010 实现、fixture 与真实双端点验收，#10 已完成 E-011 实现、真实 npm shape 与双端点全量验收，#21 已完成 D-077 的真实图闭环，#22 已在不改变依赖图的前提下完成 Docusaurus/严格 TypeScript scaffold、模块边界、受控 production build 和真实本地安装验收，#11 已完成 E-012 测试 program、runner、fixture 与远端闭环，#23 已完成统一解码、纯路径分类和项目/文章整批领域校验的远端闭环，#5 已迁移并远端关闭两份真实项目正文，#6 已完成 metadata-first 媒体清单、静态 WebP 容器/动画/尺寸/大小校验并远端闭环。#7 正在建立双模式静态素材计划与生产泄漏判定；#8、#12 至 #14 继续跟踪其余实现，production 内容装配和唯一 docs 实例仍由 #26 接管。D-078 的委托和已确认上层方向继续有效；单项本地或远端结果不能替代目标内容、完整 CI 与发布闭环。
...
| D-035 至 D-050、D-078、E-001、E-003、E-006、E-007 | 领域 schema、注册表、作者与分类引用、项目主预览、日期、可见性和 SEO 页面适配 | 领域契约、媒体字节、构建制品与浏览器 head | #23 已完成四份注册表、项目/文章和主预览引用的纯领域校验，#6 已完成媒体清单与静态 WebP 字节门禁并远端闭环；#7 正在建立目的限定的真实扫描，投影与元数据组件仍由下游实现 |
| D-053、E-008 | 按构建模式生成临时静态白名单树，并从生产制品排除未发布素材 | 源路径、白名单、制品字节和泄漏 fixture | #22 已实现空静态树的受控 production 基线；#7 继续负责真实素材白名单与泄漏检查 |
...
| D-054 至 D-060、D-078、D-079、E-006、E-012 | 单一 docs 实例、唯一判型、校验先行、只读内存投影与公共 API | 临时编译后的 Node ESM 纯逻辑测试和 Docusaurus 集成测试 | #11 已完成 Node ESM runner/fixture，#23/#6 已完成内容与媒体纯领域测试；真实扫描、只读投影和 Docusaurus 集成由 #26 实现 |
...
- `public/` 仍是迁移前手写静态入口；#22 已创建 `site-content/` 分区、根 TypeScript/Docusaurus 配置、最小 `/` 页面、模块检查器与受控 production build，#11 已实现独立 NodeNext/ES2024 测试 program，#23 已建立不读取文件系统的内容 schema/路径核心与共用结构化解码适配，#6 已建立不读取文件系统的项目媒体校验入口。#5 已在 `site-content/projects/` 创建两份真实项目正文；在 #26 接管前，I-04 构建入口必须继续以 `BUILD_PIPELINE_INCOMPLETE` 拒绝这些 Markdown，而不能静默忽略或提前发布。唯一 docs 实例、真实文件扫描、frontmatter 投影与路由装配仍由 blocked-by #7 的 #26 原子接管；不得用第二内容根、占位文档或条件 fallback 绕过。
...
- `validateArticleSource({catalog,sources})` 名称保持既定公共 API，但参数和返回值固定为整批文章源；这是 articleId/slug 唯一性、`relations.articles`、项目 `relatedWriting` 和跨文章 recommendation priority 不散落到适配层的唯一实现。每个 source 携带 `sourcePath`、结构化 frontmatter、正文及上述文件系统事实；函数稳定按 articleId 输出完整成功数组，任一文章或跨文章关系失败时不返回部分 value。UUIDv7、完整 `/writing/<lowercase-kebab-case>` slug、状态日期、作者/主题/项目/模块、关系、SEO 去重、推荐、修订和来源逐项按内容模型校验；draft 禁止 `publishedAt` 与 `revisions`，公开状态要求两日期且 `updatedAt >= publishedAt`。所有要求 string 的日期值在 YAML 中必须写成带引号的 scalar；固定 Docusaurus parser 会把未加引号的 YAML timestamp 解码为 `Date`，该非 plain 值在结构化快照边界失败，不得静默转为 ISO 字符串。首次发布日期不可改和稳定 ID 历史不可复用仍由 #12 负责；保留路由与全站冲突由 #26 负责。
...
- `src/build/static-assets/index.ts#prepareStaticAssetPlan` 是 #7 的构建层公共入口。它只接受封闭 `production|preview` 模式、已由 `validateProjectCatalog` 验证并深冻结的 `ProjectCatalog`、`docs/contracts/static-public-assets.json` 的已解码文档，以及由未来 #26 注入的未发布正文素材私有快照；返回不暴露 bytes 或未发布摘要的 `StaticAssetPlan`。计划在任何目标写入前完整扫描、验证、选择并检测全部路径冲突；其 `materialize(buildContext)` 只从计划私有快照写入已由 `readBuildContext` 验证的全新静态目录，`assertProductionBuild(buildDirectory)` 只接受 production 计划。错误使用稳定 `STATIC_ASSET_*` code、仓库相对路径和脱敏中文消息，绝对临时路径、原字节与未发布 SHA-256 不进入公开诊断。
...
- production 泄漏判定是 E-008 目的限定的静态素材 checker：它只拥有白名单逐文件路径、长度与 SHA-256 证据，未发布 path/同字节泄漏证据，以及 SSR HTML 真实图片属性引用证据。检查器安全枚举整个 `build/` 的普通文件字节，拒绝符号链接、硬链接、特殊文件、隐藏/逃逸/大小写冲突和资源上限漂移；`build/assets/projects/**` 与 production 计划逐路径、长度和 SHA-256 相等，全部白名单文件都存在且字节未变。每个公开预览必须出现在 SSR HTML 的浏览器有效 `img[src|srcset]` 候选中；`picture` 的 `source` 不能替代其必需的 `img` fallback，URL 内逗号、非法 `srcset` descriptor、`select`/`frameset`/foreign content/`noscript`/`template` 等丢弃或惰性上下文，以及注释、普通文本或 script JSON 字符串都不能假通过。未发布预览与 #26 注入的 draft 正文素材 public path 作为原始 UTF-8 byte token 跨 chunk 扫描全部制品文件，其源摘要也与每个 build 文件比较，因此改扩展名或改路径的同字节副本仍失败；preview 计划不能进入 production 泄漏判定。该 checker 不生成 release metadata、规范整树 release digest 或可部署身份，也不能替代 #33 的 release 封装与独立复验。
...
- I-04 只启用上述入口的 `production` 基线：入口验证当前 Node 是 `.nvmrc` 主端点或 `engines.node` 下界，经本地已冻结的 Docusaurus CLI 构建默认 `build/`，并创建任务私有、同用户、非宽写的临时根、空 `static/` 与随机所有权标记；`src/build/site-config/index.ts` 是根配置读取和验证该上下文的唯一公共入口。直接调用 Docusaurus、缺少或伪造环境、非临时路径、标记或 inode/权限漂移均不得回退。I-12 / #7 只把入口扩展为可验证的 production/preview 模式解析、静态素材计划与受控构建上下文；`static-public/` 或 `site-assets/` 在该计划接管前出现时以 `BUILD_PIPELINE_INCOMPLETE` 失败。真实 Markdown/MDX 内容在 #26 完成 production 内容投影与唯一 docs 实例装配前继续以同一 code 失败；`preview` Docusaurus 在 #8 接管 `build --dev`、`noindex`/sitemap 和持久候选前继续以稳定 `BUILD_MODE_UNAVAILABLE` 失败。该阶段只允许两个内容分区的 `.gitkeep` 维持物理根，不把空站点或默认 404 表述为 M0 页面完成。
- I-04 的根配置固定消费上述构建上下文，使用 canonical `url`/`baseUrl`、`trailingSlash: true`、`onBrokenLinks`/`onBrokenAnchors`/`onDuplicateRoutes: 'throw'`、`future.v4: true` 与稳定的 `future.faster: true`；显式 `blog: false`，保留 classic theme 与 sitemap，不配置搜索、分析、外部脚本或远程资源。Docusaurus 3.10.2 的 docs 插件拒绝零文档实例，而 I-04 又不得伪造公开内容或建立第二棵临时内容树，因此本阶段在 classic preset 中显式使用 `docs: false`；根 `sidebars.ts` 仍纳入严格 TypeScript 与模块边界检查，但在 #26 接管单一内容投影前不参与构建。#23 先完成纯内容门禁，#5 再创建真实项目正文；随后 #26 必须移除该临时禁用项，启用唯一 docs 实例，把 `path` 指向仓库根 `site-content/`、`routeBasePath` 设为 `/` 并使用根 `sidebars.ts`，不得以占位 Markdown、隐式默认或并行内容根绕过。固定版本会在 `.docusaurus/client-modules.js` 生成有序 CommonJS `require()` 以加载 CSS，因此根 `package.json` 不声明会把该 `.js` 改判为 ESM 的 `type` 字段；仓库 Node 入口继续通过显式 `.mjs` 保持 ESM，E-012 测试 emit 继续使用独立临时 ESM 边界。构建子进程显式禁用 Docusaurus persistent cache，不能复用 `node_modules/.cache`。`src/pages/index.tsx` 在本阶段只提供包含已确认站点名的最小 `/` 路由，使严格断链检查可以验证真实 build；它不承载首页叙事、导航、页脚或视觉完成度，也不得被表述为 M0 页面完成。语言上下文固定输出 `lang="zh-CN"`；页面、导航、页脚、视觉令牌和内容投影仍由后续 Issue 实现。
...
- D-079 只把 `@types/node@^24.0.0` 加入直接开发依赖，用途限定为 E-012 的 Node 24 测试类型；E-013 只把 `@docusaurus/utils@3.10.2` 加入直接开发依赖，用途限定为复用该精确框架版本公开导出的默认结构化 frontmatter 解析器。两者已随 #21 的真实 tarball、许可证、脚本、漏洞和完整传递图准入，不得依赖偶然提升的传递副本，也不得进入浏览器 bundle。`clsx`、`prism-react-renderer`、`@types/react-dom` 及模板、主题或未来源码中的其他包仍未获新增直接依赖授权；出现真实用途时重新准入，作为候选传递依赖出现时也不能跳过审查。除此之外的 npm 包、Docusaurus 插件、Action、浏览器 SDK、iframe、远程模块或外部服务，在完成逐项准入、记录和用户确认前不得加入。
...
- I-06 仅允许固定的 `tests/build/content-decoders-types.test.ts` 通过顶层 `import type` 精确引用 `scripts/content/frontmatter.mjs` 与 `scripts/content/json.mjs`，且相邻同名 `.d.mts` 必须是普通文件；类型导入在 emit 时消失，不形成运行时跨层依赖。`import {type ...}`、普通或 dynamic `.mjs` 导入、其他测试路径或其他脚本说明符都不属于例外并由模块门禁拒绝。
...
下列内容影响目标源码结构，必须在依赖代码创建前完成事实查证并写入对应设计。#9/#10/#21/#22/#11/#23/#5/#6 已完成各自实现与远端闭环；#7/#8、#12 至 #14 与后续装配任务继续跟踪其余实现。D-078 委托范围内的工程细节由 Agent 形成可验证决定，不再逐项请求用户确认；D-078 排除的后续外部操作或依赖变更、数据与基础设施事项仍执行用户门禁：
...
4. **部分完成**：#6 已按 E-007/CODE-008 实现主预览媒体清单、metadata-first 文件事实、静态 WebP 容器/动画/尺寸/大小门禁与公共投影并远端闭环；#7 只建立 production/preview 双模式静态素材计划、受控构建上下文和目的限定的 production 泄漏检查。#26 接管共享双模式内容扫描/投影 API、唯一 docs 实例基础装配与 production Docusaurus 接线；#8 消费该投影和 #7 的 preview 计划，接管 preview Docusaurus、持久候选与原子激活。再按 E-004 和 CODE-006 至 CODE-008 实现 React、Infima、CSS Modules 与令牌；只有浏览器 fit-gap 证据允许最小主题包装。
5. **部分完成**：#23 已按 E-006/E-007、产品字段表和 CODE-003/CODE-004 实现并远端关闭统一解码、项目/文章领域 schema、注册表、纯路径分类和稳定错误，#5 已迁移两份真实项目正文并远端关闭；#7 继续拥有媒体安全扫描与迁移，#26 拥有扫描装配、投影和构建。再按 E-013/CODE-018 实现完整历史 ledger、CI checkout 和 fixture，完成后才按 CODE-014 接线作者命令。公开业务事实和素材仍须由用户提供或确认。
...
#23 与 #5 已远端关闭并解除 #6/#7 的前置阻塞；#6 已完成 I-11 的提交、精确 SHA CI 与 Issue 关闭。其余事项未完成时，不得对外宣称目标内容、完整构建、CI、发布或生产门禁已经实现。
...
| Docusaurus/React/内容/制品/浏览器契约 | D-078、E-001 至 E-015、CODE-003 至 CODE-020 | #22 已完成无真实内容/素材的 Docusaurus build 基线，#23/#5 已完成内容领域核心与真实项目正文，#6 已完成媒体纯领域门禁并远端闭环；#7 正在建立目的限定的安全扫描，投影、主题、完整制品与浏览器验收仍待后续 Issue |
