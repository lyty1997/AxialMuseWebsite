...
- I-14 的 `SiteContentData` 展示契约只消费 `axial-muse-content-data` 插件公开的 `projectNavigation` 与 `writingNavigation`，在展示边界验证所需字段并按当前规范 pathname 查找详情；缺失、重复、未知详情或形状漂移必须抛出稳定、脱敏错误，不能退回正文字符串、原始注册表或旧页面。它不得把 articleId、sourcePath、原始关系或私有日期索引渲染为主视觉信息。
