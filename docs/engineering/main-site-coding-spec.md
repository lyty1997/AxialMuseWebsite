...
状态：active
完整度：M0-design-closed（#9/#10/#21/#22/#11/#23/#5/#6/#7/#26/#27/#28 已完成各自实现与远端验收；#8、#12 至 #14 继续跟踪）
最近更新：2026-07-27
...
2026-07-18 审查确认的实施契约矛盾由 GitHub Issues #5 至 #14 跟踪。E-006 至 E-016 已补齐内容所有权、主预览、发布态素材、草稿预览、npm 隔离、SPDX 确定化、Node ESM TypeScript 测试、完整 Git 历史门禁、同版本服务端 301、production job 字节所有权和单一内容装配；#9 已完成 E-010 实现、fixture 与真实双端点验收，#10 已完成 E-011 实现、真实 npm shape 与双端点全量验收，#21 已完成 D-077 的真实图闭环，#22 已在不改变依赖图的前提下完成 Docusaurus/严格 TypeScript scaffold、模块边界、受控 production build 和真实本地安装验收，#11 已完成 E-012 测试 program、runner、fixture 与远端闭环，#23 已完成统一解码、纯路径分类和项目/文章整批领域校验的远端闭环，#5 已迁移并远端关闭两份真实项目正文，#6 已完成 metadata-first 媒体清单、静态 WebP 容器/动画/尺寸/大小校验并远端闭环，#7 已由提交 `7f2115d9f1dc5396ca0c81fc9960223644d79725` 完成双模式静态素材计划与生产泄漏判定，CI run `29950131762` 成功并验收关闭。#26 已由提交 `91dd3c7d4b8553910418119d7ee8e677974fe01a` 完成 production 内容装配和唯一 docs 实例，精确 CI run `29970675298` 成功并验收关闭；#27 已完成页面与公开表达远端闭环，并由关闭后审查修复 `0fb38b20b8b576e53b238ab65f4ed964b52f0728` 通过精确 CI 与独立 Linux/浏览器补验；#28 已由实现提交 `255fe98f494344aca400d5a48a5d19ccb425183d` 完成主题、响应式与可访问性基线，精确 CI run `30244831021` 成功，关闭后修复 `6345bb1703000f2f72bcf4a0e71e9b960cf430ab`、`e04085b837c93db9f639ab014ad99c829b270194` 及精确 CI runs `30266019758`、`30270312057` 又完成审查缺陷收口与连续两次真实 Chrome 回归。当前 workflow 不运行该 TypeScript/浏览器入口，接线继续由 #32 跟踪。#8、#12 至 #14 继续跟踪其余实现。D-078 的委托和已确认上层方向继续有效；单项闭环不能替代目标内容、完整 CI 与发布闭环。
...
| D-074 | Docusaurus 目标源码使用严格 TypeScript，类型检查与静态构建相互独立 | `tsc --noEmit` 与 Docusaurus build | #22 已实现生产 TypeScript program、独立 typecheck 与最小 build；目标 CI 尚未接线 |
...
| D-052、D-053、D-073、D-077、D-079、E-010、E-011、E-014、E-015 | 依赖准入、锁文件、隔离冻结安装、确定性供应链证据、质量、production job 字节闭包和带 301 配置的发布必需门禁 | 依赖、制品、浏览器与发布检查 | E-010/E-011 与 #21 真实图准入、正式证据及双端点 composite receipt 已完成；301、自包含重建、浏览器制品检查和目标 CI 接线尚未完成 |
...
下列是 2026-07-23 可从工作区查证的迁移事实，不代表目标工程已经就绪或已经提交到远端：
...
- Docusaurus 目标源码必须独立运行 `tsc --noEmit`，Docusaurus build 必须独立验证框架加载和静态制品；两项都是发布必需门禁，任一成功都不能替代另一项。#22 已把两个本地入口接入隔离 `quality`/`build` 并在任务临时副本真实通过；目标 GitHub CI 迁移尚未完成，不能由本地结果替代远端 required check。
...
- 目标质量门禁还必须失败关闭执行 D-077 的候选来源、tarball/integrity、许可证与脚本、SPDX/NOTICE 漂移、显式 audit、双端点冻结安装和 manifest/lock 前后不变检查。仓库 `quality` 已包含只读静态供应链闭包，#21 的受限 audit/双端点/composite receipt 证明首次真实图本地通过；目标 GitHub CI 仍须把同一静态负载和受限 artifact retention 接线，不能以本地回执替代远端 required check。
...
- #26 的 production 制品 checker 只能在 Docusaurus 内建 build 子进程对 `.axial-muse-build-candidate-<owner>` 完整退出 0 后，由外层受控入口以独立 `axial-muse:check-production` 命令创建全新 session、重新扫描并串行调用；插件 `postBuild` 因与 sitemap 并行且早于最终断链结论，只能写入私有日期索引与 build-phase 输入摘要，不得承担最终通过判定。输入摘要必须以域分隔 framing 同时绑定完整内容批次，以及静态计划的模式、全部 allowed/excluded 条目语义、字节长度和 SHA-256；不得只绑定当前 production 会复制的公开素材。checker 必须精确核对 owner 绑定的完整输入摘要与私有日期索引，并把实际 HTML route、每页唯一 title/description/canonical/Open Graph、`zh-CN`、单一 H1、固定导航页脚、真实空状态或公开列表/详情显示字段、项目图片单次投影、尾斜杠、sitemap、真实 doc sidebar 叶链接、未发布 token、私有索引的最终制品缺席和 E-008 静态计划与同一重新验证的内容投影闭合；静态页和无图片文章不得输出 `og:image`，项目图片只能来自同一已验证主预览。checker 的稳定文件读取在 operation-only 或 close-only 时直接保留唯一原错误对象；两者同时失败时使用 `AggregateError([operationError, closeError])`，固定主操作在前且不重试 close，外层 code、message 与受控相对路径保持不变。
...
下列内容影响目标源码结构，必须在依赖代码创建前完成事实查证并写入对应设计。#9/#10/#21/#22/#11/#23/#5/#6/#7/#26/#27/#28 已完成各自实现与远端闭环；#8、#12 至 #14 与后续任务继续跟踪其余实现。D-078 委托范围内的工程细节由 Agent 形成可验证决定，不再逐项请求用户确认；D-078 排除的后续外部操作或依赖变更、数据与基础设施事项仍执行用户门禁：
...
| #21 供应链策略与真实图闭包 | `check-supply-chain.mjs`、候选/生成/audit/双端点/最终编排入口及 `tests/build/supply-chain-*.test.mjs` | 策略/admissions、真实候选报告、正式三制品、audit 全零、D-082 决定、双端点与 composite receipt 已通过本地验收；目标 CI 与受限 artifact retention 待接线 |
...
| 严格 TypeScript 与独立类型检查/构建 | `tsconfig.json`、`typecheck`、受控 `build` 与真实冻结安装验收 | #22 已自动化生产基线并真实通过；目标 CI 尚未接线 |
| 模块目录、公共入口、导出与别名边界 | `check-module-boundaries.mjs` 与 `module-boundaries.test.mjs` | 已自动化，当前 16 个正常/反例测试通过；后续源码继续纳入同一检查器 |
| 首轮直接依赖与生产/测试 `tsconfig` 基线 | D-076、D-079、E-012 设计契约 | 首轮依赖、lock、生产/测试 `tsconfig`、runner 和配置漂移检查已完成；目标 CI 迁移仍由后续任务接管 |
| 首次依赖解析与供应链准入 | D-077、CODE-015 与 #21 | 1,345 个物理依赖/1,225 个 canonical identity、正式三制品、audit 全零、最终决定和双端点 composite receipt 已完成本地闭环；远端 CI 成功证据尚未形成 |
| Node ESM TypeScript 测试 | D-079、E-012、CODE-005/CODE-011/CODE-016 | #11 已完成 runner、fixture、主/最低 Node 同负载与现有远端 CI 验收；目标 Node 24 workflow 迁移尚未实现 |
...
| Docusaurus/React/内容/制品/浏览器契约 | D-078、E-001 至 E-016、CODE-003 至 CODE-020 | #22 已完成无真实内容/素材的 Docusaurus build 基线，#23/#5 已完成内容领域核心与真实项目正文，#6/#7 已完成媒体与目的限定安全扫描并远端闭环，#26 已远端闭环单一 docs 内容投影、侧栏、日期索引和 production 制品检查，#27 已远端闭环页面与公开表达，#28 已远端闭环主题、响应式与浏览器最终验收 |
