...
- ACME 客户端自动签发并续期覆盖 `axialmuse.com` 与 `www.axialmuse.com` 的证书。
...
生产服务器不承担写作、构建或源码编辑。Node.js 质量检查在 GitHub Actions 完成；服务器只取已经通过门禁的精确提交并发布其中的 `public/`。
...
4. 从目标 SHA 导出 `public/` 到临时目录，不执行仓库内任意脚本。
5. 校验 `index.html`、CSS、关键锚点、文件权限和目录大小。
...
- `http://axialmuse.com/*` 跳转到 `https://www.axialmuse.com/*`。
- `http://www.axialmuse.com/*` 跳转到 `https://www.axialmuse.com/*`。
...
首版优先使用 ACME 客户端自动签发双域名证书，并启用 systemd 定时续期与 deploy hook：

1. DNS 已指向服务器且 80/443 放通。
2. 首次签发同时包含 `axialmuse.com` 和 `www.axialmuse.com`。
3. 每日由 ACME timer 检查是否需要续期。
4. 续期后执行 `nginx -t`，成功才 reload。
5. 每日外部检查证书剩余有效期，低于 21 天告警。

每个项目子域名单独签发单主机证书，不使用泛域名证书，也不把所有项目加入主站证书。项目证书续期失败不得阻塞主站或其他项目 reload。

不把证书或私钥提交到 Git。腾讯云免费证书当前有效期为 90 天，快速续期相当于重新申请且重新部署；对自管 Nginx 而言，自动化 ACME 更符合无人值守维护目标。参考：[腾讯云免费证书续期](https://cloud.tencent.com/document/product/400/61353)、[Certbot 文档](https://eff-certbot.readthedocs.io/en/stable/)。
...
5. 安装并验证 Nginx、Git、ACME 客户端与 TAT agent。
