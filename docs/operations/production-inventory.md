...
| Build command | 仓库已实现 `build`、`package:artifact` 与 `check:artifact`；后两者复用冻结依赖中的 production checker，并完成确定性封装及外部 `releaseContentSha256` 复验。#14 已由专题提交 `a9a2f29` 把完整顺序接入四个 prerequisite 之后的 `production-artifact` job，并随 topic `e88386b` 普通推送，但尚无 canonical `main` 真实运行 |
...
| 服务器 artifact 校验器 | #35 已由专题提交 `4bbf859` 实现只使用 Python 3.12 标准库的 `ops/deploy/verify_artifact.py` 与 Node/Python 共享 golden，并随 topic `e88386b` 普通推送；#36 工作区另有一次性 root bootstrap 候选，但两者都尚未进入 canonical `main`，本地负向验收不等于服务器安装。2026-07-29 已现场确认 `/usr/bin/python3` 为 root-owned 链接并精确解析到兼容的 root-owned Python 3.12；安装目标固定为 `/usr/local/lib/axialmuse/artifact-verifier/` 的 root-owned verifier/golden，bootstrap 只作经摘要认证的瞬时执行器，不安装自身。须待 bootstrap、verifier 与 golden 同一源提交进入 canonical `main`，再以精确提交和三项摘要完成独立认证并取得现场安装授权；#14 真实 ZIP shape 复验仍待 |
| 跨 job build 边界 | E-015 固定不上传或下载中间 build artifact；`production-artifact` 在 fresh runner 自包含重建、重验和封装，避免消费其他 job 文件系统 |
| 发布新鲜度 | #34 已由专题提交 `4f6de96` 在活动 workflow 外实现七项 producer outputs、当前 GitHub context、canonical main/run/artifact/head/digest 的两次 main 交叉核验，以及 main 不取消/生产串行的静态 concurrency 候选，并随 topic `e88386b` 普通推送；尚未接入真实 workflow/API，concurrency 仍不替代新鲜度 |
| Workflow token | prerequisite 与 #14 producer 的 `contents: read` 已由专题提交 `a9a2f29` 配置，其他 producer scope 为 `none`；#34 专题提交 `4f6de96` 的静态 deploy fixture 只允许 `contents: read`、`actions: read`，两者均已随 topic `e88386b` 普通推送，但尚未写入活动 workflow 或真实 environment |
...
| Web、TLS、TAT 与系统运行时 | 旧静态站点已删除；D-124 当时的独立软件后验证明无未知非本地 TCP 监听、Nginx/TAT active、failed unit 为零，Ubuntu 官方 Certbot 七包、CLI/timer 和无 ACME 状态通过。D-129 新 boot 的 diagnostic-only remainder 又在两条间隔 10 秒的新会话中一致证明 Nginx、Certbot、监听、failed units 与本地 TAT 当前语义门禁通过；原正式完整后验仍因 vendor/cross-reboot posture 契约 pending。用户已在 D-130 人工确认当前腾讯云控制面正常；D-132 首轮 formal 在继承的 `ExecStart` 形态门禁失败且未形成 receipt。D-133 当前分类确认 TAT 为 `single-zero` 且 unit/fragment/process 门禁均为真，unattended-upgrades 为 `numeric-pair` 且 unit/fragment 门禁均为真；D-134 已把该差异编码为严格 unit-aware v2 本地候选，这两步本身均不构成 formal acceptance。后续获准的一轮 v2 formal 已在同一 boot 的两条 fresh SSH 会话中间隔至少 10 秒完成，所有 gates 均为真，结果为 `status=accepted-with-residuals`、`vendorState=fully_absent`、`aggregateDisposition=accepted_residual_unattributed`，且 `serverMutationPerformed=false`。Python 3.12 前置运行时已盘点；verifier bootstrap 只有工作区候选，没有现场安装证据 |
...
| #36 证据与重启门禁 | D-125 正式隔离重建回执按当时 schema 继续保留 `environmental_inconclusive`；TEST-NET 重放支持 initial-enable 到 persisted reload/cold-start 时序作为 103/105 差异的强结构候选。D-126 以唯一一次获准的服务器只读语义变换探针得到 `status=complete oracleMatch=true`，只证明 transition 前的当前内容关系；D-128 在独立授权下执行 component-aware transition 并得到 `status=complete outcome=committed`。D-129 又在独立授权下只执行一次重启；授权已消费，结果因 vendor declaration predicate 不满足而保持 pending。D-130 新建的只读逐组件语义验收源码/契约已本地审计；D-132 获准的一轮 formal 以 `remote-posture-exec-start-shape` 失败，没有 receipt 或自动重试，也不改写 D-129。D-133 随后以一次同 boot 隐私安全分类确认当前 TAT `single-zero` 与 unattended-upgrades `numeric-pair` 形态及对应布尔门禁，但不构成 formal receipt。D-134 已本地升级 unit-aware v2 policy/schema、receipt 路径与正反例；后续获准的一轮 v2 formal 已生成 mode `0600`、被 Git 忽略的本地私有 receipt，并完成独立校验：两条 fresh SSH 会话位于同一 boot、间隔至少 10 秒，所有 gates 均为真，结果为 `status=accepted-with-residuals`、`vendorState=fully_absent`、`aggregateDisposition=accepted_residual_unattributed`、`serverMutationPerformed=false`。该结果不改写 D-129/D-132，也不追溯证明历史因果；除正常 SSH/sudo 审计副作用外，本轮没有服务器或云资源写操作。#36 只剩 bootstrap、verifier 与 golden 进入 canonical `main` 后的 verifier 现场安装；第二次重启、服务恢复、再基线、云规则变更或 #37 仍须另行授权 |
...
| 软件与 Certbot 基线 | D-124 精确完成六包升级与七包新增，零额外、零移除；独立软件后验确认全部十三包、APT 标记、Certbot CLI/官方 timer、无 ACME 状态及既有 SSH/UFW/YJ/Nginx/TAT/监听边界 | 2026-08-02 | 该回执形成时尚未重启、签发证书或安装 verifier；D-129 diagnostic remainder 已通过当前软件语义门禁，D-129 原 formal 保持 pending；D-132 首轮 D-130 formal 失败且没有 receipt。D-133 分类与 D-134 本地契约本身不替代 formal，但后续获准的 unit-aware v2 formal 已形成并独立校验 `accepted-with-residuals` 私有 receipt；verifier 仍未现场安装 |
...
| D-130 只读逐组件 semantic acceptance | 用户确认当前主机侧无已知问题且腾讯云控制面正常；新契约只接受 vendor `fully_absent` 或原严格 `rejecting` 完整态，保留其余逐组件门禁，并把旧 pre/post 聚合不等显式记为 `accepted_residual_unattributed` | 2026-08-04 | D-132 首轮 formal 以 `remote-posture-exec-start-shape` 失败且未生成 receipt；D-133 当前分类收窄到 TAT `single-zero` outer normalizer。D-134 随后把本地候选升级为 unit-aware v2：TAT 仅单值零、unattended-upgrades 仅数字对、Nginx 既有内层规则不扩大，并以整串 grammar、policy/schema、v1/v2 receipt 隔离、live binding 与 mutation 反例完成本地审计。后续获准的一轮 v2 formal 在同一 boot 的两条 fresh SSH 会话中间隔至少 10 秒通过全部 gates，生成并独立校验 mode `0600`、被 Git 忽略的私有 receipt；结果为 `status=accepted-with-residuals`、`vendorState=fully_absent`、`aggregateDisposition=accepted_residual_unattributed`、`serverMutationPerformed=false`，且不改写 D-129/D-132 |
| 生产冒烟 | 未执行 | - | - |
| 桌面端渲染 | 未执行 | - | - |
| 移动端渲染 | 未执行 | - | - |
| 恢复演练 | 未执行 | - | - |
...
| 日期 | 变更 | 原因 | 验证 |
|---|---|---|---|
| 2026-08-04 | 执行 D-130 unit-aware v2 formal 只读验收 | 以 D-134 已审计的严格逐 unit 契约取得 D-129 之后的独立当前态证据，同时保留 D-129/D-132 历史结论 | 两条 fresh SSH 会话位于同一 boot、间隔至少 10 秒且所有 gates 均为真；本地私有、被 Git 忽略且 mode `0600` 的 v2 receipt 已生成并独立校验，结果为 `status=accepted-with-residuals`、`vendorState=fully_absent`、`aggregateDisposition=accepted_residual_unattributed`、`serverMutationPerformed=false`。除正常 SSH/sudo 审计副作用外，没有服务器或云资源写操作；#36 只剩 canonical `main` 后的 verifier 现场安装，#37 继续暂停 |
| 2026-08-04 | 按 D-133 执行一次隐私安全只读 ExecStart 分类 | 在不输出原始现场字段或修改验收契约的边界内，把 D-132 的两个 outer-normalizer 候选收窄到可审计的当前形态 | 远端连接前修复首轮复审发现的尾部误分类；最终候选通过固定依赖、同 boot、challenge、干净环境、命令白名单和正反例审计。一次新 SSH 会话确认 TAT 为 `single-zero` 且 unit/fragment/process 门禁均为真，unattended-upgrades 为 `numeric-pair` 且 unit/fragment 门禁均为真。没有 receipt、重试或服务器/云写入；契约修正与 formal 重跑仍待分别授权 |
| 2026-08-04 | 按 D-134 本地升级 D-130 unit-aware v2 契约 | 只把 D-133 已分类的 TAT 单值零作为该 unit 的严格展示等价值；其他 unit 不全局放宽 | v2 候选显式传入 unit，整串匹配固定字段顺序并拒绝未知 unit、交叉 status、缺失/重复/乱序/额外字段、第二记录和尾随内容；policy、remote/local schema、receipt 路径及 mutation 证据同步升级。仅执行本地审计，没有服务器连接、formal、receipt 或 Git 发布 |
| 2026-08-03 | 按 D-132 执行一轮 D-130 formal 并失败关闭 | 以新的逐组件契约取得 D-129 之后的独立只读验收证据 | 本地审计通过后进入同一新 boot 的正式只读流程；语义观测返回 `remote-posture-exec-start-shape`，未生成 receipt、未自动重试。没有服务器或云控制面写入、第二次重启、reload、补链、再基线、恢复、verifier 或 #37；具体 unit 归因、契约修正和再次 formal 均待新授权 |
