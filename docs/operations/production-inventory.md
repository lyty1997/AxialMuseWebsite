...
| Web 与 TLS 组件 | 既有旧静态站点经用户确认后已删除；D-124 软件后验再次确认 Nginx 配置测试成功、active 且没有活动 server block 或 TCP 80/443 监听。Ubuntu 官方 Certbot CLI 与 timer 已安装并通过独立后验，timer enabled/active/waiting；没有 ACME 账户、证书或 lineage，本轮未签发或读取证书。精确版本与其他服务姿态不写入公开仓库 |
| SSH、账户与系统防火墙 | 额外交互账户已归因为首启用户数据创建，并于 2026-07-30 按 D-118 移除唯一受控直接 sudo 规则、设置账户过期和不可交互 shell，同时保留账户/home/密钥及其他文件。D-119/D-120 随后以可回退事务完成 root-owned SSH 全局策略，独立严格公钥会话与最终后验达到 `committed_clean`；既有管理身份、端口、主机密钥、主密钥文件和 sudo 路径未变。D-122 又在目标服务器提交并启用 UFW：默认拒绝入站、允许出站、禁用 routed，只允许已验证开发机来源访问既有 SSH 端口和任意 IPv4 来源访问 TCP 80/443，没有 IPv6 用户放行或其他入站，既有腾讯 YJ 链保持不变；第二条严格 SSH、精确临时状态清理和重启前稳态均通过。详细身份、来源和规则正文不写入公开仓库 |
| 腾讯云入站边界 | 用户已手工允许任意 IPv4 来源访问 TCP 80/443；SSH 方案 A 删除额外来源后曾连续三次 TCP 22 建连超时，随后用户恢复此前三条受限云 SSH 规则集合。严格 SSH 已恢复，服务器连接元数据确认实际来源仍对应槽位 2，并与 OS UFW 唯一 SSH 来源规则匹配；云层当前仍是恢复后的三来源集合，尚未再次收敛为单一来源，不增加 IPv6 或其他入站 |
...
| 系统更新 | 只读核验窗口内观察到当前管理身份另行发起并完成系统包升级，用户于 2026-07-30 确认是本人有意执行；关键服务与配置终态复核通过。Ubuntu 官方自动安全更新服务及两个 apt timer 已启用，未配置自动重启。D-124 于 2026-08-02 以单一事务精确升级 `distro-info-data`、`libssl-dev`、`libssl3t64`、`openssl`、`tzdata`、`tzdata-legacy` 六包，并新增七个 Certbot 最小包，零额外、零移除且未执行 `autoremove`；当时的独立软件后验通过。云 SSH 恢复后的完整 `post-software` 在最终 `live-hash` 失败；随后冻结 capture 证明配置、包与 boot 摘要仍和旧证据一致。D-125 正式回执继续为 `environmental_inconclusive`，最早 full-hash 漂移仍因历史组件证据缺失而不能严格归因；其后的 D-126 一次性只读语义变换探针返回 `status=complete oracleMatch=true`，仅证明当前规范化 non-vendor 投影加两条公开标准 IPv6 rate-limit 终止规则后等于冻结 oracle。当前加固态快照正常且恢复通道可用；没有执行维护重启、服务器写入、UFW reload 或重新建立基线，component-aware transition 仍待新的决定与授权 |
...
| SSH、管理身份与双层防火墙 | 额外交互身份的创建来源和闲置状态已经查明，D-118 可逆禁用及独立后验已经完成；D-119/D-120 SSH 全局策略已通过两条独立严格公钥会话和最终后验并达到 `committed_clean`。D-122 OS UFW 已按最小 IPv4 边界提交，第二条严格 SSH、提交状态清理及重启前稳态均通过；云层 TCP 80/443 已开放给任意 IPv4。方案 A 失联后用户已恢复此前三条受限云 SSH 规则，严格 SSH 成功，服务器实际来源仍对应槽位 2 且匹配 OS UFW 唯一规则。D-125 正式回执继续为 `environmental_inconclusive`；D-126 返回 `status=complete oracleMatch=true`，闭合了当前 non-vendor 内容与冻结 oracle 的两规则变换关系，但不证明历史因果。云层单来源收敛、component-aware transition 与维护重启仍暂停，恢复后的诊断没有执行维护重启、服务器写入、UFW reload、重新建立基线或再次修改云规则。精确地址、端口与规则正文只保留在私密现场记录 |
| Web、TLS、TAT 与系统运行时 | 旧静态站点已删除且 Nginx 当前无活动 server block 或 Web 监听；独立软件后验仍没有未知非本地 TCP 监听，Nginx/TAT 均 active 且 failed unit 为零。Python 3.12 前置运行时已盘点，TAT 控制面 online、无未知任务；Ubuntu 官方 Certbot 七包已经最小安装，CLI 与官方 timer 后验通过且没有 ACME 状态。verifier bootstrap 只有工作区候选，没有现场安装证据 |
| 系统补丁与重启 | 最近系统包升级后的关键服务和配置复核通过，用户确认升级是本人有意执行；D-117 降为历史恢复点，当前 D-122 加固态软件事务前快照已确认正常且恢复通道可用。Ubuntu 官方自动安全更新服务和两个 apt timer 已启用，未配置自动重启；D-124 精确六包升级与七包新增及当时的独立软件后验已经完成，`reboot-required` 仍存在。恢复 SSH 后的完整 `post-software` 在最终 `live-hash` 失败；D-125 正式结果为 `environmental_inconclusive`，随后 D-126 的 `oracleMatch=true` 只闭合当前内容关系，没有恢复旧维护重启授权。维护重启完全没有执行，也没有重新建立基线；须先取得 component-aware transition 的明确决定与授权，完成云层单来源收敛和复验后，才可另行执行重启与 SSH/TAT/Nginx/UFW/Certbot/systemd/监听/包状态稳态验收 |
| 当前恢复点 | 用户确认 D-122 加固态软件事务前系统盘快照在腾讯云控制台为正常，控制台或 TAT 恢复通道可用；未执行恢复演练，恢复或删除仍须另行授权 |
| D-125/D-126 证据与重启门禁 | D-125 正式隔离重建回执按当时 schema 继续保留 `environmental_inconclusive`；TEST-NET 重放支持 initial-enable 到 persisted reload/cold-start 时序作为 103/105 差异的强结构候选。D-126 随后以唯一一次获准的服务器只读语义变换探针得到 `status=complete oracleMatch=true`：当前规范化 non-vendor 加两条公开标准规则精确命中冻结 oracle。该结果不追溯证明历史因果，也不授权 transition、重启、reload、重新建立基线、verifier、云规则变更或 #37；授权已经消费 |
| 腾讯云控制面 | 用户于 2026-07-30 提供脱敏逐项结论：实例/生命周期、快照能力/配额、恢复入口、TAT/任务、代理保留和账号保护未发现阻断偏差；2026-08-02 确认 Web IPv4 入站已开放。方案 A 删除额外 SSH 来源并失联后，用户已恢复此前三条受限规则集合；严格 SSH 成功，服务器实际来源仍是槽位 2 且与 OS UFW 唯一 SSH 来源匹配。云规则没有再次修改；D-126 的当前内容关系结论不授权云规则变更，因此云层单一来源收敛继续暂停。另有一条历史出带宽告警无法确定归因；用户按 D-116 接受仅限该单次告警的残余不确定性，未来复发或持续异常仍重新失败关闭 |

第一阶段结论仍为 **GAP / 失败关闭**，不是 #36 完成证据。旧静态站点、控制面基础核验、当前 D-122 加固态恢复点、额外管理身份的可逆禁用、D-119/D-120 SSH 全局策略、D-122 OS 防火墙重启前稳态、云 Web 入站，以及 D-124 精确软件事务和当时的独立后验已经收敛，TAT 与腾讯云代理按用户决定保留。用户恢复此前三条受限云 SSH 规则后，严格 SSH 已成功，实际来源仍对应槽位 2 并匹配 OS UFW 唯一规则；但完整 `post-software` 在最终 `live-hash` 失败。D-125 正式隔离重建结果为 `environmental_inconclusive`，不能追溯证明服务器漂移或差异唯一来自环境。其后的 D-126 唯一正式探针返回 `status=complete oracleMatch=true`，证明当前规范化 non-vendor 投影加入两条公开标准 IPv6 rate-limit 终止规则后等于冻结 oracle；探针前后未执行服务器写入、UFW reload、维护重启或重新建立基线。该结果闭合当前内容关系，但不改变 D-125 的历史结论，也不证明最早 full-hash 差异的历史因果。D-126 授权已经消费；必须先取得 component-aware transition 的新决定和对应授权，再把云 SSH 收敛为单一来源并复验，在此之前禁止重启、任何 verifier、云规则变更和 #37。

在上述正式回执之后，D-125 又开展了不连接服务器、不读取真实配置或使用真实地址的 TEST-NET 补充重放。UFW 0.36.2 源码顺序与非敏感等价配置连续两次复现 initial-enable 103、reload 105：首次 `enable` 先加载当时没有 IPv6 用户规则的 `user6.rules`，logging 随后才把两个标准 IPv6 rate-limit 终止规则写入持久配置，后续 reload/cold-start 才加载它们。等价配置中 IPv4 投影不变、IPv6 只新增这两个规则，IPv6 user input/output/forward 为空，且没有 jump/goto 进入对应 limit 链，因此新增规则在该等价配置下不可达。这只支持首次启用时序作为当前 103/105 mismatch 的强结构候选；重放结果与正式回执仅完成逐链数量绑定，不能证明真实规则内容 exact 一致。现有冻结 transaction/evidence 已按契约清理真实输入，仅余不可逆 hash/摘要，无法再恢复输入完成本机 exact 重放；该路径已经失败关闭，正式 `environmental_inconclusive` 和最早 full-live 摘要的组件缺失残余继续保留。D-126 随后执行了唯一一次获准的只读语义变换探针并返回 `status=complete oracleMatch=true`，闭合当前规范化 non-vendor 与冻结 oracle 的两规则变换关系；它没有传出配置、规则或摘要值，也没有写服务器。该结果不追溯证明历史因果；下一步仍须就 component-aware transition 作出新决定并授权，确认前全部服务器和实现动作暂停。
...
| OS 防火墙 | D-122 已提交 UFW 最小 IPv4 入站边界，完成第二条严格 SSH、事务临时状态精确清理及重启前稳态验收；既有腾讯 YJ 链保持不变 | 2026-08-01 | 脱敏现场验收；开发机本地 UFW 与腾讯云控制面均未操作 |
| 软件与 Certbot 基线 | D-124 精确完成六包升级与七包新增，零额外、零移除；独立软件后验确认全部十三包、APT 标记、Certbot CLI/官方 timer、无 ACME 状态及既有 SSH/UFW/YJ/Nginx/TAT/监听边界 | 2026-08-02 | 脱敏现场验收；尚未重启、签发证书或安装 verifier |
| 云 SSH 管理通道 | 用户恢复此前三条受限规则集合后严格 SSH 成功；服务器实际来源仍对应槽位 2，并与 OS UFW 唯一 SSH 来源规则匹配 | 2026-08-02 | 完整 `post-software` 在最终 `live-hash` 失败；D-126 已闭合当前 non-vendor 内容与冻结 oracle 的两规则变换关系，但未授权重新建立基线、重启或修改 UFW/云规则。云层单来源收敛仍暂停 |
